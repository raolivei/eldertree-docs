---
id: NET-003
title: Fix Nodes After Netplan Configuration Issue
category: network
severity: high
source: pi-fleet/docs/FIX_NODES_AFTER_NETPLAN.md
symptoms:
  - See Error Messages section below
---

# NET-003: Fix Nodes After Netplan Configuration Issue
...
Both nodes (node-1 and node-1) are unreachable due to incorrect Netplan static IP configuration.
...
# Check hostname
hostname
# Should show: node-1.eldertree.local or node-1.eldertree.local
```
...
- node-1: `node-1.eldertree.local`, IP: `192.168.2.80`
...
2. **Static IP auto-calculation** - IPs automatically assigned: node-1 = 192.168.2.80, node-1 = 192.168.2.81
